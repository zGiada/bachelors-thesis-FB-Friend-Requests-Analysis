...
def selectCity():
    time.sleep(2)
    try:
        #save the div's text content where there's the hometown
        capture = driver.find_element_by_xpath("//div[@class='dati1w0a tu1s4ah4 f7vcsfb0 discj3wi']").text
        time.sleep(2)
        with open('copy2.txt', 'w') as wf:
            txt = (capture.encode('utf-8', errors='ignore'))            
            wf.write(txt + "\n")
            wf.close()
        find = False
        file = open('copy2.txt', 'r')
...
def selectOccupation():
    try:
        elems = driver.find_element_by_xpath("//div[@class='dati1w0a tu1s4ah4 f7vcsfb0 discj3wi']")
        #print("\nPersona:")
        try:
            elem = driver.find_elements_by_xpath("//div[@class='tu1s4ah4']")
            i=0
            study = False
            work = False
            for e in elem:                
                if i == 0:
                    #print(e.text)
                    if (("Nessun" not in e.text) and ("tudent" not in e.text) and ("cuola" not in e.text)) or (("avor" in e.text) and ("Nessun" not in e.text)):
                        work = True
                    else:
                        work = False
                if i == 1:
                    if work:
                        ...
                        if "Nessun" not in e.text:
                            study = True
                            break
                        else:
                            if ("requenta" or "tudia") in elems.text:
                                study = True
                            else:
                                study = False
                i=i+1
            ...
            if work:
                if ("isoccupat" in elems.text) or ("ensionat" in elems.text):
                    occupation = "N"
                else:
                    occupation = "W"

            if ((not work) & (not study)):
                occupation = "T"
...
def classificator(url,gender,real_img,age):
    ...
    with open("../victims.json", "r+") as f:
        ...
        x = 0
        stop = False
<<<<<<< HEAD:periodo2/code-level1/search-profile/classificator-profile2.py
        if len(temp) < 40:
=======
        if len(temp) < 500:
>>>>>>> periodo4:code-final_version/search-profile/classificator-profile2.py
            alreadyExist = False
            while x < len(temp):
                ind = temp[x]
                if ind in url:
